...
import CategoryPageServices from './../../../Services/Customer/CategoryPageServices';
import { useEffect } from 'react';
import { useState } from 'react';
import { ICategory, ISubCategory, IProduct } from '../../../Services/Interfaces/Interfaces';
...
const CategoryPage = () => {
    ...
    // filtered
    const [subCategoryFiltered, setSubCategoryFiltered] = useState<number>(0);
    ...
    const handleSubCategoryFilterChange = (
        ...
        setSubCategoryFilter(parseInt(e.target.value, 10));
    };
    ...
    const handleFilterSubmit = async (e: React.FormEvent<HTMLFormElement>, urlName: string, subCat: number, price: string, color: number, sort: string) => {
        e.preventDefault();
        setSubCategoryFiltered(subCat);
            ...
            <div className="container my-4">
                ...
                <div className="mb-3">
                    ...
                    <div className="collapse mb-3" id="collapseExample">
                        <form onSubmit={(e) => handleFilterSubmit(e, urlName || '', subCategoryFilter || 0, priceFilter || '', colorFilter || 0, sortFilter || '')}>
                                ...
                                <div className="col">
                                    <div className="card rounded-0 hide-border-right" style={{ fontSize: 12 }}>
                                        ...
                                        <div className="card-body p-0 custom-scrollbar px-2" style={{ maxHeight: 150, height: 150, overflow: 'auto' }}>
                                            {subCategories && subCategories.map((x, index) => (
                                                <span key={index}>
                                                    <input
                                                        id={x.urlName}
                                                        name="subCat"
                                                        ...
                                                        className="d-none"
                                                        value={x.id}
                                                        checked={subCategoryFilter === x.id}
                                                        onChange={(e) => handleSubCategoryFilterChange(e)}
                                                    />
                                                    <label
                                                        htmlFor={x.urlName}
                                                        className={`btn btn-outline-dark rounded-0 mt-2 me-2 
                                                    ${subCategoryFilter === x.id ? 'active' : ''}`}
                                                    >
                                                        {x.name}
                                                    </label>
                                                </span>
